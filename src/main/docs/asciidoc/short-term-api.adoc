== 단기예보 저장 및 조회 API
기상청 OPEN API에서 단기예보 데이터를 얻어와 데이터베이스에 저장합니다.

== 단기예보 저장

=== 요청구조
include::{snippets}/short-term/save/http-request.adoc[]

== 요청 파라미터
include::{snippets}/short-term/save/request-fields.adoc[]
include::{snippets}/short-term/save/request-body.adoc[]

* `regionCode` - 예보가 요청되는 지리적 지역을 나타내는 고유 코드입니다. 자세한 코드는 https://docs.google.com/spreadsheets/d/1wrFcpkXZLwSQzImgrvZnTkUcA6_gM4au/edit?usp=sharing&ouid=113873634025863855868&rtpof=true&sd=true[지역 코드 표]를 참조하세요.
* `baseDate` - 예보 데이터가 요청되는 날짜입니다. YYYYMMDD 형식이어야 합니다.
* `baseTime` - 예보 데이터가 요청되는 시간입니다. 허용되는 값은 0200, 0500, 0800, 1100, 1400, 1700, 2000, 2300입니다.

=== 응답결과
예보 데이터 저장 성공 시 HTTP 상태 200 OK를 반환합니다.
include::{snippets}/short-term/save/http-response.adoc[]

== 단기예보 조회
인자로 받은 `regionCode`, `baseDate`, `baseTime` 매개변수를 기반으로 데이터베이스에서 JSON 형식의 데이터를 가져옵니다.

=== HTTP 요청
include::{snippets}/short-term/get/http-request.adoc[]

=== 요청 예시
[source,http]
----
GET /api/weather-forecast/short-term/details?regionCode=110&baseDate=20240101&baseTime=0800
----

=== 응답 파라미터
include::{snippets}/short-term/get/response-fields.adoc[]

=== 응답예시
include::{snippets}/short-term/get/response-body.adoc[]
